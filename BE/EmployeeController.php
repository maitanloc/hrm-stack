...
class EmployeeController extends Controller
{
    ...
    public function profileShow(Request $request, array $params): array
    {
        ...
        return $this->ok([
            'employee' => $employee,
            'employment_histories' => $this->employees->listEmploymentHistories($id, 40),
            'certificates' => $this->employees->listCertificates($id),
            'certificate_types' => $this->employees->listCertificateTypes(),
    ...
    public function employmentHistory(Request $request, array $params): array
    {
        ...
        return $this->ok(
            $this->employees->listEmploymentHistories($id, 80),
    ...
    public function certificateIndex(Request $request, array $params): array
    {
        ...
        return $this->ok([
            'items' => $this->employees->listCertificates($id),
            'types' => $this->employees->listCertificateTypes(),
        ], 'Certificate list');
    }
    ...
    public function certificateStore(Request $request, array $params): array
    {
        ...
        $certificateId = $this->employees->createCertificate($id, $payload);
        $items = $this->employees->listCertificates($id);
        $created = null;
        foreach ($items as $row) {
            if ((int) ($row['certificate_id'] ?? 0) === $certificateId) {
                $created = $row;
                break;
            }
        }
        return $this->created($created, 'Certificate created');
    }
    ...
    public function certificateDelete(Request $request, array $params): array
    {
        ...
        if ($certificateId <= 0) {
            throw new HttpException('certificateId is required', 422, 'validation_error');
        }

        $this->employees->deleteCertificateById($id, $certificateId);
