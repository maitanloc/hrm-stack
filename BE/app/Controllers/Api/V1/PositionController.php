...
class PositionController extends Controller
{
    ...
    public function publicCatalog(Request $request): array
    {
        $paging = Paginator::resolve($request);
        $search = $request->query('q');
        $status = $request->query('status');
        ...
        $items = array_map(static function (array $item): array {
            return [
                ...
                'position_group' => (string) ($item['position_group'] ?? ''),
                'position_level' => (string) ($item['position_level'] ?? ''),
            ];
        }, $result['items']);

        return $this->ok(
            $items,
            'Public position catalog',
            Paginator::meta($result['total'], $paging['page'], $paging['per_page'])
        );
    }
