...
class LeaveController extends Controller
{
    ...
    public function leaveRequestUpdate(Request $request, array $params): array
    {
        ...
        $currentStatus = $this->normalizeRequestStatus((string) ($currentRequest['status'] ?? 'CHỜ_DUYỆT'));
    ...
    private function resolveLeaveApprovalStatus(array $roleCodes, string $currentStatus, float $leaveDays): ?string
    {
        ...
        if ($isManager && $currentStatus === 'CHỜ_DUYỆT') {
            return 'ĐANG_XỬ_LÝ';
        }
        ...
        if ($isHr && in_array($currentStatus, ['ĐANG_XỬ_LÝ', 'CHỜ_XÁC_NHẬN_HR'], true)) {
            return 'ĐÃ_DUYỆT';
        }
