...
class Position extends Model
{
    ...
    public function findBestByName(string $positionName): ?array
    {
        ...
        $exactSql = "SELECT p.*
                     FROM positions p
                     WHERE LOWER(TRIM(p.position_name)) = LOWER(TRIM(:position_name))
                     ORDER BY p.position_id DESC
                     LIMIT 1";
        $stmt = $this->db->prepare($exactSql);
        ...
        $likeSql = "SELECT p.*
                    FROM positions p
                    WHERE LOWER(p.position_name) LIKE LOWER(:position_like)
                    ORDER BY p.position_id DESC
                    LIMIT 1";
        $stmt = $this->db->prepare($likeSql);
        $stmt->execute(['position_like' => '%' . $name . '%']);
        $row = $stmt->fetch();
