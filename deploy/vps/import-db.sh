...
compose exec -T mysql mysql --default-character-set=utf8mb4 -uroot "-p${MYSQL_ROOT_PASSWORD}" -e "CREATE DATABASE IF NOT EXISTS HRM_SYSTEM CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"

echo "Import base schema/data..."
compose exec -T mysql mysql --default-character-set=utf8mb4 -uroot "-p${MYSQL_ROOT_PASSWORD}" HRM_SYSTEM < "SQL_hackathon v4.sql"
compose exec -T mysql mysql --default-character-set=utf8mb4 -uroot "-p${MYSQL_ROOT_PASSWORD}" HRM_SYSTEM < "data.sql"
...
for f in \
  ...
  20260331_attendance_second_session_columns.sql
do
  compose exec -T mysql mysql --default-character-set=utf8mb4 -uroot "-p${MYSQL_ROOT_PASSWORD}" HRM_SYSTEM < "BE/scripts/$f"
done
